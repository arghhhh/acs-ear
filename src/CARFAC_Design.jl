...
for ch = 1:n_ch
  ...
  pole_Hz = pole_Hz - CF_CAR_params.ERB_per_step * ERB_Hz(pole_Hz, CF_CAR_params.ERB_break_freq, CF_CAR_params.ERB_Q)
end
...
for e = 1:n_ears
	...
	ears[e].SYN_coeffs = SYN_coeffs;
        ...
        CF = CARFAC()
        CF.fs                      = fs
        CF.max_channels_per_octave = max_channels_per_octave
        CF.CAR_params              = CF_CAR_params
        CF.AGC_params              = CF_AGC_params
        CF.IHC_params              = CF_IHC_params
        CF.SYN_params              = CF_SYN_params
        CF.n_ch                    = n_ch
...
function CARFAC_DesignFilters(CAR_params, fs, pole_freqs)
        ...
        CAR_coeffs.a0_coeffs  = zeros(n_ch)
        CAR_coeffs.c0_coeffs  = zeros(n_ch)
        CAR_coeffs.h_coeffs   = zeros(n_ch)
        CAR_coeffs.g0_coeffs  = zeros(n_ch)
        CAR_coeffs.ga_coeffs  = zeros(n_ch)
        CAR_coeffs.gb_coeffs  = zeros(n_ch)
        CAR_coeffs.gc_coeffs  = zeros(n_ch)
        CAR_coeffs.OHC_health = ones(n_ch)  # % 0 to 1 to derate OHC activity.
        ...
        if min_zeta <= 0  # % Use this to do a new design strategy
                ...
                min_zetas = min_zeta .+ 0.25*(ERB_Hz.(pole_freqs, CAR_params.ERB_break_freq, CAR_params.ERB_Q) ./ pole_freqs .- min_zeta)
                CAR_coeffs.r1_coeffs = (1 .- theta .* max_zeta) # % "r1" for the max-damping condition
                CAR_coeffs.zr_coeffs = theta .* (max_zeta .- min_zetas) # % how r relates to undamping
        end
...
function CARFAC_DesignAGC(AGC_params, fs, n_ch)
        ...
        mix_coeffs = zeros(n_AGC_stages) # was row vector
        ...
        AGC_coeffs.n_AGC_stages        = n_AGC_stages
        AGC_coeffs.n_ch                = n_ch
        AGC_coeffs.decimation          = AGC_params.decimation
        AGC_coeffs.AGC_stage_gain      = AGC_params.AGC_stage_gain
        AGC_coeffs.mix_coeffs          = mix_coeffs
        AGC_coeffs.feedback_gains      = zeros(n_AGC_stages) # was row vector
        ...
        for stage = 1:n_AGC_stages
                ...
                if !FIR_OK
                        error( "AGC 3-point FIR design failed; try less decimation, or less spread or delay, or longer time constant.")
                end
...
function Design_FIR_coeffs(delay_variance, mean_delay)
...
function CARFAC_DesignIHC(IHC_params, fs, n_ch)
        ...
        elseif IHC_params.one_cap
                ...
                IHC_state = IHC_state_struct()
                IHC_state.cap_voltage            = IHC_coeffs.rest_cap
                IHC_state.lpf1_state             = 0
                IHC_state.lpf2_state             = 0
                IHC_state.ihc_accum              = 0
        else
                ...
                IHC_coeffs.out2_rate        = r2min / (IHC_params.tau2_out * fs)
                ...
                # % one-channel state for testing/verification:
                IHC_state = IHC_state_struct()
                IHC_state.cap1_voltage     = IHC_coeffs.rest_cap1
                IHC_state.cap2_voltage     = IHC_coeffs.rest_cap2
                IHC_state.lpf1_state       = 0
                IHC_state.lpf2_state       = 0
                IHC_state.ihc_accum        = 0
        end
