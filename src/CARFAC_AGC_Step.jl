...
#                        @show size(coeffs.feedback_gains)
#                        @show size(state.AGC_memory)
#                        @show size(coeffs.input_gains)
#                        @show size(AGC_in)
#                        @show size(coeffs.next_stage_gains)
#                        @show size(state.AGC_memory[:, 2:end])
#                        @show size(zeros(coeffs.n_ch))
#                        @show size(hcat( state.AGC_memory[:, 2:end], zeros(coeffs.n_ch) ))


                        state.AGC_memory = ( # % Broadcasting gain rows across channels
                                  coeffs.feedback_gains' .* state.AGC_memory 
                                + coeffs.input_gains' .* AGC_in 
                          #      + coeffs.next_stage_gains .* [state.AGC_memory(:, 2:end), zeros(coeffs.n_ch, 1)]
                                + coeffs.next_stage_gains' .* hcat( state.AGC_memory[:, 2:end], zeros(coeffs.n_ch) )
                        )
                        ...
                        r,c = size( state.AGC_memory )

#                        @show size(coeffs.spatial_FIR)
#                        @show  coeffs.spatial_FIR[1, :]
#                        @show  coeffs.spatial_FIR[2, :]
#                        @show  coeffs.spatial_FIR[3, :]
#                        @show size(state.AGC_memory)

#                        @show r c
                        
#                        @show size(coeffs.spatial_FIR[1, :])
#                        @show size(state.AGC_memory[ vcat( 1, 1:(r-1)), : ] )
#                        @show coeffs.spatial_FIR[1, :]' .* state.AGC_memory[ vcat( 1, 1:(r-1)), : ]


                        state.AGC_memory = ( # % Mix in neighbors, replicating edges.
                             #     coeffs.spatial_FIR(1, :) .* state.AGC_memory([1, 1:(end-1)], :) 
                             #     coeffs.spatial_FIR[1, :] .* state.AGC_memory[ [1, 1:(end-1)], : ]
                                  coeffs.spatial_FIR[1, :]' .* state.AGC_memory[ vcat( 1, 1:(r-1)), : ]
                                + coeffs.spatial_FIR[2, :]' .* state.AGC_memory[ :, : ]
                             #   + coeffs.spatial_FIR[3, :] .* state.AGC_memory[ [2:end, end], : ]
                                + coeffs.spatial_FIR[3, :]' .* state.AGC_memory[ vcat( 2:r, r ), : ]
                        )
                        updated = true # 1;  % Updates on every call.
                else
                        updated = false # 0;
                end
        else
...
function CARFAC_AGC_Recurse(coeffs, AGC_in, stage, state)
        ...
        if decim_phase == 0
                ...
                AGC_stage_state = 
                          coeffs.feedback_gains[stage] * AGC_stage_state + coeffs.input_gains[stage] * AGC_in
                if stage < coeffs.n_AGC_stages
                        ...
                        state,_ = CARFAC_AGC_Recurse(coeffs, AGC_in, stage+1, state) # % RECURSE
                        # % The coefficient for adding in the next stage state is premultiplied
                        # % by the AGC_epsilon.
                        AGC_stage_state = AGC_stage_state + coeffs.next_stage_gains[stage] * state.AGC_memory[:, stage+1]
                end
                # % spatial smooth:
                FIR_coeffs = coeffs.spatial_FIR[:, stage]

                # work around MATLAB logical indexing / my understanding of this syntax....


        #        @show AGC_stage_state size( AGC_stage_state )
        #        dump( AGC_stage_state ; maxdepth=1 )

                (r,) = size( AGC_stage_state )

        #        @show r
        #        @show FIR_coeffs  
        #        @show FIR_coeffs[1]  
        #        @show AGC_stage_state
        #        @show [1, 1:(r-1)]
        #        @show vcat( 1, 1:(r-1) )
        #        @show AGC_stage_state[ vcat( 1, 1:(r-1) ) ]
#
        #        @show FIR_coeffs[1] * AGC_stage_state[ vcat( 1, 1:(r-1) ) ]
        #        @show FIR_coeffs[2] * AGC_stage_state
        #        @show FIR_coeffs[3] * AGC_stage_state[ vcat(2:r, r) ]
                AGC_stage_state = (
                        #  FIR_coeffs[1] * AGC_stage_state[ [1, 1:(end-1)], : ] 
                          FIR_coeffs[1] * AGC_stage_state[ vcat( 1, 1:(r-1) ) ] 
                        + FIR_coeffs[2] * AGC_stage_state 
                        # + FIR_coeffs[3] * AGC_stage_state[ [2:end, end], : ]
                        + FIR_coeffs[3] * AGC_stage_state[ vcat(2:r, r) ]
                )
                # % Finally store the state back (could do it all in-place).
                state.AGC_memory[:, stage] = AGC_stage_state

                updated = true # 1;  % bool to say we have new state; only used from stage 1.
        else
                updated = false # 0;
        end
