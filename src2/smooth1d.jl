...
# specific version of filter for the case being used here.
# this works on multiple channels in columns
function filter1p( b0, a1, x, state = zeros( size(x,2) ) )
        y = similar(x)
        for i in 1:size(x,1)

        #        @show size( b0 * x[i,:] ) size( a1 * state )
                y[i,:] = b0 * x[i,:] - a1 * state
                state = y[i,:]
        end
...
@show size(cols) scale
    ...
    #                    #[nr, nc, nl] = size(cols);
    #            else
    #                    error("error in shape passed to smooth1d")
    #            end
    #    end
        ...
        #        [cols, state] = filter(1-polez, [1, -polez], cols, state);
                 ...
                 (x   , state) = filter1p(1-polez, -polez, cols);
                 cols = cols[end:-1:1,:];
                 (cols, state) = filter1p(1-polez, -polez, cols, state);
                 cols = cols[end:-1:1,:];
                 (cols, state) = filter1p(1-polez, -polez, cols, state);
