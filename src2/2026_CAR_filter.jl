...
mutable struct CAR_filter <: Processors.SampleProcessor
        ...
        function CAR_filter( f_ch, fs, zero_ratio = sqrt(2), high_f_damping_compression = 0.5, min_zeta = 0.1, max_zeta = 0.35 )
...
function Processors.process( f::CAR_filter, x1, state = (; z1_memory=0.0, z2_memory=0.0 ) )
    ...
    # x_in,undamping = x1
        ...
        return (; y, r, z1, z2, ideal_g ),( z1_memory=z1, z2_memory=z2 )
end
...
function Processors.process( f::Constant_undamping, x, state=nothing )
        return (;in=x,undamping=f.undamping), nothing
end
