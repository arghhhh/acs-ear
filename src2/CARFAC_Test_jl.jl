...
figures = Plots.Plot[] # these are the ones from the MATLAB version
figures_extra = Plots.Plot[]  # extra plots that seem useful
...
# end  % Produce plots by default.
	...
	# reset figures list:
	global figures       = Plots.Plot[]
	global figures_extra = Plots.Plot[]

        # % Run tests, and see if any fail (have nonzero status):
        status = false  # % 0 for OK so far; 1 for test fail; 2 for error.
        status = status | test_CAR_freq_response(do_plots)                   # fig 1
        status = status | test_IHC1(do_plots) # % one_cap, v1                # fig 2, 3
        status = status | test_IHC2(do_plots) # % two_cap, v2                # fig 4, 5
        status = status | test_IHC3(do_plots) # %  do_syn, v3                # fig 6, 7, 8, 9
        status = status | test_AGC_steady_state(do_plots)                    # fig 10
        status = status | test_AGC_steady_state_simpler_decimating(do_plots) # fig 11
        status = status | test_AGC_steady_state_non_decimating(do_plots)     # fig 12
        status = status | test_stage_g_calculation(do_plots)                 # fig 13
        status = status | test_whole_carfac1(do_plots)                       # fig 14, 15, 16
        status = status | test_whole_carfac2(do_plots)                       # fig 17, 18, 19
        status = status | test_whole_carfac3(do_plots)                       # fig 20, 21, 22
        status = status | test_whole_carfac1_non_decimating(do_plots)        # fig 23, 24, 25
        status = status | test_whole_carfac2_non_decimating(do_plots)        # fig 26, 27, 28
        status = status | test_whole_carfac3_non_decimating(do_plots)        # fig 29, 30, 31
        status = status | test_delay_buffer(do_plots)                        # NO FIGURES
        status = status | test_OHC_health(do_plots)                          # fig 32
        status = status | test_multiaural_silent_channel(do_plots)           # NO FIGURES
        status = status | test_multiaural_silent_channel_simpler_decimating(do_plots) # NO FIGURES
        status = status | test_multiaural_silent_channel_non_decimating(do_plots) # NO FIGURES
        status = status | test_multiaural_carfac(do_plots)                   # NO FIGURES
        status = status | test_spike_rates(do_plots)                         # fig 33, 34, 35
...
#=
	for (i,f) in enumerate(figures)
		@show i
		savefig( f, "../png/julia2/julia-figure-$(i).svg" )
	end
	for (i,f) in enumerate(figures_extra)
		@show i
		savefig( f, "../png/julia2/julia-figure-extra-$(i).svg" )
	end
=#
...
#=
        ...
        complex_spectra1 = fft(impulse_responses,1)  # do FFT column wise - MATLAB style
=#

	impulse = zeros( n_points )
	impulse[1] = 1.0

	r = impulse |> CF.ears[1].CAR_coeffs |> CollectArrays
	r1 = permutedims( hcat( r, zeros( size(r) ) ), (2,1) )
	complex_spectra = fft( r1, 1 )

#	@show maximum( abs.( complex_spectra - complex_spectra1 ) )
		...
		push!( figures_extra, plot( db_spectra, xlim=(10,2^14), ylim=(-20,65), xaxis=:log, legend=nothing  ) )
		push!( figures_extra, plot( db_spectra[:,1:5:end], xlim=(10,2^14), ylim=(-20,65), xaxis=:log, legend=nothing  ) )
        ...
        for i = 1:size(expected, 1)
                # channel needs to be an int type because it is used as an index below:
                channel = round( Int, expected[i, 1] ) + 1 # % Channel number was 0 based from Python.
                correct_cf = expected[i, 2]
                correct_gain = expected[i, 3]
                correct_bw = expected[i, 4]
                correct_q = expected[i, 5]
                cf_amp_bw = find_peak_response(spectrum_freqs, db_spectra[:, channel], 3) # % 3 dB width
                ...
                gain = round(cf_amp_bw[2] ; digits=2 )
                bw = round(cf_amp_bw[3] ; digits=1)
                q = round(cf_amp_bw[1] / cf_amp_bw[3] ; digits= 1)
                @printf "%d: cf is %.1f Hz, peak gain is %.1f dB, 3 dB bandwidth is %.1f Hz (Q = %.1f).\n'" channel cf gain bw q
                if cf != correct_cf
                        status = 1
                        @printf "Mismatch cf %f should be %f.\n" cf correct_cf
                end
                if gain != correct_gain
                        status = 1
                        @printf "Mismatch gain %f should be %f.\n" gain correct_gain
                end
                if bw != correct_bw
                        status = 1
                        @printf "Mismatch bw %f should be %f.\n" bw correct_bw
                end
                if q != correct_q
                        status = 1
                        @printf "Mismatch q %f should be %f.\n" q correct_q
                end
        end
...
function test_whole_carfac1(do_plots)
	status = test_whole_carfac(do_plots, :one_cap, false);
...
function test_whole_carfac(do_plots = false, version_string = :one_cap, non_decimating = false )
	# % Test: Make sure that the AGC adapts to a tone.
	# % Test with open-loop impulse response.

	# do_plots=false ; version_string = :one_cap ; non_decimating = false
...
# % Test: Verify frequency dependent reduced gain with reduced health.
	...
	rms(v) = sqrt.( Statistics.mean( x->x*x, v; dims=1) )

	fs = 22050;
	t = (0:(1/fs):(1 - 1/fs));  # % Sample times for 1s of noise
	amplitude = 1e-4;  # % -80 dBFS, around 20 or 30 dB SPL
	noise = amplitude * randn(length(t),1);

	# CF = CARFAC_Design(1, fs, 'one_cap');  # % Legacy
	CF = CARFAC_Design_version( :one_cap, 1, fs );  # % Legacy


	CF_state_ears = CARFAC_Init(CF);
	#[~, CF, bm_baseline] = CARFAC_Run_Segment(CF, CF_state_ears, noise);
	(; CF, CF_state_ears, BM ) = CARFAC_Run_Segment(CF, CF_state_ears, noise);
	bm_baseline = BM

	#half_ch = floor(CF.n_ch/2);
	half_ch = div(CF.n_ch,2);
	ch = 1:half_ch;
	CF.ears[1].CAR_coeffs.OHC_health[ch] = CF.ears[1].CAR_coeffs.OHC_health[ch] * 0.5;
	CF_state_ears = CARFAC_Init(CF);
	# [~, CF, bm_less_healthy] = CARFAC_Run_Segment(CF, CF_state_ears, noise);
	(; CF, CF_state_ears, BM ) = CARFAC_Run_Segment(CF, CF_state_ears, noise);
	bm_less_healthy = BM

	@show size(bm_baseline)

	# BM has an extra dimension at the ends - ears?

	# these end up as row vectors (matrices) same as MATLAB version
	rms_baseline = rms(bm_baseline[:,:,1]);
	rms_less_healthy = rms(bm_less_healthy[:,:,1]);
	...
	#	drawnow
		p = plot()
                push!( figures, p )
		plot!( tf_ratio' )
		plot!( xlabel="channel number", ylabel="tf_ratio" )
		plot!( title="unhealthy hf OHC noise transfer function ratio" )
	end
...
function find_closest_channel(values, desired)
	index = argmin((values .- desired).^2);
	return index
end
...
# if nargin < 3, extra = 0; end
        if extra
                if status
                        println( "FAIL ", name, "; at least one test failed." )
                else
                        println( "PASS ", name, "; all tests passed." )
                end
        elseif status
                println( "FAIL ", name )
        #        if status > 1
